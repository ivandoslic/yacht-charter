  ...
  return (
    <div className='flex w-full gap-1 md:gap-5 flex-col md:flex-row justify-between'>
        <Dropdown
            ...
            selectedOption={categoryState}
            setSelectedOption={setCategoryState}
            onSelect={(category) => onCategoryChange(category)}
        />
        <TextInput
            placeholder="Number of guests"
            icon={<UserIcon class="h-4 w-4 md:h-10 md:w-8"/>}
            dataType="number"
            textState={guestsState}
            setTextState={setGuestsState}
        />
        <TextInput
            placeholder="Number of cabins"
            icon={<InboxIcon class="h-4 w-4 md:h-10 md:w-8"/>}
            dataType="number"
            textState={cabinsState}
            setTextState={setCabinsState}
        />
